...
import {
  ...
  type ChapterApproval, type ChapterReviewTimeline, type ExportEstimate, type ExportFormat, type ExportPackage, type Issue, type Job, type KokoroSetupStatus, type LocalAiCatalogItem, type LocalAiInstallJob, type ReadinessReport,
...
export function ProjectDashboard() {
  ...
  async function approveSelectedChapter() { if (!selectedProjectId || !selectedChapter) return; setSectionWorking("approval", true); try { const approval = await approveChapter(selectedProjectId, selectedChapter.id, { approvedBy: "local-user" }); setChapterApproval(approval); setReadiness(await runReadiness(selectedProjectId, selectedChapter.id)); setIssues(await listIssues(selectedProjectId)); setNotice("Chapter marked listened and approved."); } catch (cause) { setError(messageOf(cause)); } finally { setSectionWorking("approval", false); } }
  async function patch(issue: Issue) { if (!selectedProjectId || !issue.segmentId) return; try { await patchSegment(selectedProjectId, issue.segmentId, { issueId: issue.id }); setNotice("Segment patched and its chapter reassembled."); if (selectedChapter) { await refreshProduction(selectedProjectId, selectedChapter.id); await refreshReviewTimeline(selectedProjectId, selectedChapter.id); await refreshChapterApproval(selectedProjectId, selectedChapter.id); } await inspectSegment(issue.segmentId); setReadiness(await runReadiness(selectedProjectId, selectedChapter?.id)); setIssues(await listIssues(selectedProjectId)); } catch (cause) { setError(messageOf(cause)); } }
  ...
  function followWorkflowAction(action: WorkflowAction) { setActiveSection(action.step); if (action.segmentId) void inspectSegment(action.segmentId); if (action.issueId) { const issue = issues.find((item) => item.id === action.issueId); if (issue) void openIssue(issue); } }
