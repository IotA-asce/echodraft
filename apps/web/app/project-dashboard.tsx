...
export function ProjectDashboard() {
  ...
  async function extract() { if (!selectedProjectId) return; setBusy(true); try { const extraction = await extractStructure(selectedProjectId); await waitFor(extraction.id, selectedProjectId); const [nextChapters, nextWarnings, nextQuality, nextCharacters, nextAttributions] = await Promise.all([listChapters(selectedProjectId), listStructureWarnings(selectedProjectId), getStructureQuality(selectedProjectId), listCharacters(selectedProjectId), listSpeakerAttributions(selectedProjectId)]); setChapters(nextChapters); setStructureWarnings(nextWarnings); setStructureQuality(nextQuality); setCharacters(nextCharacters); setSpeakerAttributions(nextAttributions); setActiveSection("structure"); setNotice("Structure and cast draft extracted. Review cast and voices before producing chapters."); } catch (cause) { setError(messageOf(cause)); } finally { setBusy(false); } }
