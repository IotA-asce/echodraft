...
export function ProjectDashboard() {
  ...
  async function patch(issue: Issue) { if (!selectedProjectId || !issue.segmentId) return; try { await patchSegment(selectedProjectId, issue.segmentId, { issueId: issue.id }); setNotice("Segment patched and its chapter reassembled."); if (selectedChapter) await refreshProduction(selectedProjectId, selectedChapter.id); await inspectSegment(issue.segmentId); } catch (cause) { setError(messageOf(cause)); } }
