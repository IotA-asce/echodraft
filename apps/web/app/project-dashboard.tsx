...
import {
  ...
  extractStructure,
  getChapterApproval,
  getChapterReviewTimeline,
  getJob,
  getKokoroSetup,
  getLocalAiInstallJob,
  getProductionSettings,
  getProductionStatus,
  getSegmentReviewInspector,
  getSource,
  getStructureQuality,
  getTtsProviders,
  getTtsSettings,
  ...
  listPronunciations,
  listRenderQueue,
  inferSegmentDirections,
  listLocalAiCatalog,
  listProjects,
  listScenes,
  listSegmentDirections,
  listSegments,
  listSoundAssets,
  listSourcePages,
  listSpeakerAttributions,
  listStructureWarnings,
  ...
  previewVoice,
  produceChapter,
  rejectCharacterMerge,
  reparseSource,
  runReadiness,
  runSpeakerAttribution,
  saveProductionSettings,
  saveSegmentDirection,
  saveSegmentOverride,
  saveTtsSettings,
  testTtsSettings,
  setStructureLock,
  ...
  type ChapterReviewTimeline,
  type ExportEstimate,
  type ExportFormat,
  type ExportPackage,
  type Issue,
  type Job,
  type KokoroSetupStatus,
  type LocalAiCatalogItem,
  type LocalAiInstallJob,
  ...
  type SegmentRenderComparison,
  type SegmentReviewInspector,
  type SoundAsset,
  type SoundCue,
  type SourceDocument,
  type SourcePage,
  type SpeakerAttribution,
  type StructureParserWarning,
  type StructureQuality,
  type TextCleanlinessIssue,
  type TtsProvider,
...
const messageOf = (cause: unknown) =>
  cause instanceof Error
    ? cause.message
    : "The local studio could not complete that request.";
...
export function ProjectDashboard() {
  ...
  const [cleaningIssues, setCleaningIssues] = useState<TextCleanlinessIssue[]>(
    [],
  );
  const [structureWarnings, setStructureWarnings] = useState<
    StructureParserWarning[]
  >([]);
  const [structureQuality, setStructureQuality] =
    useState<StructureQuality | null>(null);
  ...
  const [selectedTtsProvider, setSelectedTtsProvider] =
    useState<TtsProvider>("mock");
  const [kokoroSetup, setKokoroSetup] = useState<KokoroSetupStatus | null>(
    null,
  );
  ...
  const [structureJob, setStructureJob] = useState<Job | null>(null);
  const [localAiCatalog, setLocalAiCatalog] = useState<LocalAiCatalogItem[]>(
    [],
  );
  const [localAiJob, setLocalAiJob] = useState<Job | null>(null);
  const [localAiInstallJob, setLocalAiInstallJob] =
    useState<LocalAiInstallJob | null>(null);
  const [issues, setIssues] = useState<Issue[]>([]);
  const [comments, setComments] = useState<Comment[]>([]);
  const [activeIssue, setActiveIssue] = useState<Issue | null>(null);
  const [exports, setExports] = useState<ExportPackage[]>([]);
  const [exportEstimate, setExportEstimate] = useState<ExportEstimate | null>(
    null,
  );
  const [characters, setCharacters] = useState<Character[]>([]);
  const [speakerAttributions, setSpeakerAttributions] = useState<
    SpeakerAttribution[]
  >([]);
  ...
  const [renderCompare, setRenderCompare] =
    useState<SegmentRenderComparison | null>(null);
  const [segmentInspector, setSegmentInspector] =
    useState<SegmentReviewInspector | null>(null);
  const [reviewTimeline, setReviewTimeline] =
    useState<ChapterReviewTimeline | null>(null);
  const [chapterApproval, setChapterApproval] =
    useState<ChapterApproval | null>(null);
  ...
  const [exportAudioVariant, setExportAudioVariant] = useState<
    "active" | "clean" | "mixed"
  >("active");
  const [exportTitle, setExportTitle] = useState("");
  const [exportAuthor, setExportAuthor] = useState("");
  const [exportAlbum, setExportAlbum] = useState("");
  const [exportPublisher, setExportPublisher] = useState("");
  const [exportLanguage, setExportLanguage] = useState("en");
  const [exportCoverPath, setExportCoverPath] = useState("");
  const [characterName, setCharacterName] = useState("");
  const [characterAliases, setCharacterAliases] = useState("");
  const [characterTraits, setCharacterTraits] = useState("");
  const [soundAssetType, setSoundAssetType] = useState<
    "ambience" | "music" | "sfx"
  >("ambience");
  const [soundCueType, setSoundCueType] = useState<
    "ambience" | "music" | "sfx"
  >("ambience");
  const [soundRenderMode, setSoundRenderMode] = useState<
    "light" | "dramatized" | "all"
  >("light");
  const [soundGain, setSoundGain] = useState(-24);
  ...
  const narrator =
    voices.find((item) => item.id === production?.narratorVoiceProfileId) ??
    null;
  const kokoroVoices = kokoroSetup?.availableVoices.length
    ? kokoroSetup.availableVoices
    : tts?.provider === "kokoro"
      ? tts.availableVoices
      : [];
  const activeProviderInfo =
    ttsProviders.find((item) => item.provider === selectedTtsProvider) ?? null;
  const currentSceneId = segments[0]?.sceneId ?? scenes[0]?.id ?? null;
  const activeSoundAsset =
    soundAssets.find((item) => item.id === selectedSoundAssetId) ??
    soundAssets.find((item) => item.assetType === soundCueType) ??
    soundAssets[0] ??
    null;
  ...
  const activeStructureJob =
    structureJob && ["queued", "running"].includes(structureJob.status)
      ? structureJob
      : null;
  const activeStructureJobPercent = activeStructureJob
    ? jobProgressPercent(activeStructureJob)
    : null;
  const terminalStructureJob =
    structureJob && ["failed", "cancelled"].includes(structureJob.status)
      ? structureJob
      : null;
  ...
  useEffect(() => {
    ...
    getTtsSettings()
      .then((next) => {
        setTts(next);
        setSelectedTtsProvider(next.provider);
      })
      .catch((cause) => setError(messageOf(cause)));
    getTtsProviders()
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void estimateExport(selectedProjectId, "wav", selectedExportIds, {
      audioVariant: exportAudioVariant,
      title: exportTitle.trim() || undefined,
      author: exportAuthor.trim() || undefined,
      album: exportAlbum.trim() || undefined,
      publisher: exportPublisher.trim() || undefined,
      language: exportLanguage.trim() || undefined,
      coverImagePath: exportCoverPath.trim() || undefined,
    })
      .then((estimate) => {
        if (!cancelled) setExportEstimate(estimate);
      })
      .catch(() => {
  ...
  async function loadProject(
    projectId: string,
    options: { activate?: WorkflowStepId | null } = {
    ...
    const settled = await Promise.allSettled([
      ...
      getProductionSettings(projectId),
      listIssues(projectId),
      listExports(projectId),
      listCharacters(projectId),
      listSpeakerAttributions(projectId),
      listSegmentDirections(projectId),
      listPronunciations(projectId),
    ...
    setSource(nextSource.status === "fulfilled" ? nextSource.value : null);
    setChapters(nextChapters.status === "fulfilled" ? nextChapters.value : []);
    setStructureWarnings(
      nextWarnings.status === "fulfilled" ? nextWarnings.value : [],
    );
    setStructureQuality(
      nextQuality.status === "fulfilled" ? nextQuality.value : null,
    );
    if (nextSource.status === "fulfilled") {
      listSourcePages(nextSource.value.id)
        .then(setSourcePages)
        .catch(() => setSourcePages([]));
      listCleaningIssues(nextSource.value.id)
        .then(setCleaningIssues)
        .catch(() => setCleaningIssues([]));
    } else {
      setSourcePages([]);
      setCleaningIssues([]);
    }
    if (nextVoices.status === "fulfilled") setVoices(nextVoices.value);
    if (nextProduction.status === "fulfilled")
      setProduction(nextProduction.value);
    if (nextIssues.status === "fulfilled") setIssues(nextIssues.value);
    if (nextExports.status === "fulfilled") setExports(nextExports.value);
    if (nextCharacters.status === "fulfilled")
      setCharacters(nextCharacters.value);
    if (nextAttributions.status === "fulfilled")
      setSpeakerAttributions(nextAttributions.value);
    if (nextDirections.status === "fulfilled")
      setSegmentDirections(nextDirections.value);
    if (nextPronunciations.status === "fulfilled")
      setPronunciations(nextPronunciations.value);
    if (nextSoundAssets.status === "fulfilled")
      setSoundAssets(nextSoundAssets.value);
    const restoredStructureJob =
      nextStructureJobs.status === "fulfilled"
        ? nextStructureJobs.value[0]
        : null;
  ...
  async function waitFor(jobId: string, projectId: string) {
    for (let i = 0; i < 80; i += 1) {
      const next = await getJob(jobId);
      if (next.status === "succeeded") return;
      if (next.status === "failed" || next.status === "cancelled")
        throw new Error(next.errorMessage || "Background task failed.");
      await new Promise((resolve) => setTimeout(resolve, 250));
    }
    throw new Error("The task is taking longer than expected.");
  }
  async function refreshLocalAi() {
    try {
      setLocalAiCatalog(await listLocalAiCatalog());
    } catch (cause) {
      setError(messageOf(cause));
    }
  }
  ...
  async function refreshExportEstimate(projectId: string) {
    try {
      setExportEstimate(
        await estimateExport(projectId, "wav", selectedExportIds, {
          audioVariant: exportAudioVariant,
          title: exportTitle.trim() || undefined,
          author: exportAuthor.trim() || undefined,
          album: exportAlbum.trim() || undefined,
  ...
  async function refreshImportedSource(projectId: string) {
    const nextSource = await getSource(projectId);
    setSource(nextSource);
    setSourcePages(await listSourcePages(nextSource.id).catch(() => []));
    setCleaningIssues(await listCleaningIssues(nextSource.id).catch(() => []));
  }
  ...
  async function refreshTtsSetup(options?: { syncProvider?: boolean }) {
    ...
    setTts(nextTts);
    setKokoroSetup(nextSetup);
    setTtsProviders(nextProviders);
    if (options?.syncProvider || nextTts.provider === "kokoro")
      setSelectedTtsProvider(nextTts.provider);
  }

  async function create(event: FormEvent) {
    event.preventDefault();
    if (!title.trim() || !rights) return;
    setBusy(true);
    try {
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    setActiveSection("manuscript");
    try {
      const imported = await importSource(selectedProjectId, file);
      setImportJob(imported);
      setNotice(
        "Manuscript import is running locally. Large PDFs and OCR can take a few minutes.",
      );
      if (imported.status === "succeeded") {
        ...
        setActiveSection("manuscript");
        setBusy(false);
      } else if (!["queued", "running"].includes(imported.status))
        setBusy(false);
    } catch (cause) {
      setError(messageOf(cause));
      setBusy(false);
  ...
  async function extract() {
    ...
    setStructureJob(null);
    try {
      const extraction = await extractStructure(selectedProjectId);
      setStructureJob(extraction);
      setActiveSection("structure");
      setNotice(
        "Structure and cast draft is running locally. This can take several minutes with local LLM enabled.",
      );
      if (extraction.status === "succeeded") {
        ...
        setBusy(false);
      } else if (!["queued", "running"].includes(extraction.status))
        setBusy(false);
    } catch (cause) {
      setError(messageOf(cause));
      setBusy(false);
  ...
  async function saveEdit() {
    ...
    setBusy(true);
    try {
      const updated = await updateSegment(editing.id, draft.trim());
      setSegments((current) =>
        current.map((item) => (item.id === updated.id ? updated : item)),
      );
      setEditing(null);
      setNotice(
        `Revision r${updated.revision} saved. It will be rendered during the next chapter run.`,
      );
      if (selectedProjectId && selectedChapter)
        await refreshProduction(selectedProjectId, selectedChapter.id);
      if (segmentInspector?.segment.id === updated.id)
        await inspectSegment(updated.id);
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
      setBusy(false);
  ...
  async function toggleSegmentLock(segment: Segment) {
    try {
      const updated = (await setStructureLock("segment", segment.id, {
        ...
        reason: segment.userLocked ? null : "Locked in Structure Editor",
      })) as Segment;
      setSegments((current) =>
        current.map((item) => (item.id === updated.id ? updated : item)),
      );
  ...
  async function splitAtReadingBreak(segment: Segment) {
    ...
    const splitOffset = before > 24 ? before : after > 0 ? after : midpoint;
    try {
      await splitSegment(segment.id, splitOffset);
      setSegments(await listSegments(segment.sceneId));
      setSegmentInspector(null);
      setNotice(
        "Segment split. Review both resulting segments before production.",
      );
    } catch (cause) {
      setError(messageOf(cause));
    }
  ...
  async function saveTts() {
    ...
    try {
      const saved = await saveTtsSettings({
        ...
        setupMode:
          selectedTtsProvider === "kokoro"
            ? tts.setupMode
            : selectedTtsProvider === "piper"
              ? "local_cli"
              : selectedTtsProvider === "xtts_v2"
                ? "coqui_local"
                : null,
        executable: tts.executable,
        runtimeRoot: tts.runtimeRoot,
        pythonPath: tts.pythonPath,
        modelPath: tts.modelPath,
        voicesDataPath: tts.voicesDataPath,
        voiceRegistryPath: tts.voiceRegistryPath,
        piperModelPath: tts.piperModelPath,
        piperConfigPath: tts.piperConfigPath,
        referenceVoicePath: tts.referenceVoicePath,
        referenceVoiceConsent: tts.referenceVoiceConsent,
        language: tts.language,
      });
      setTts(saved);
      setSelectedTtsProvider(saved.provider);
      await testTtsSettings();
      await refreshTtsSetup({ syncProvider: true });
      setNotice("Local voice engine settings saved and validated.");
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
      setBusy(false);
  ...
  async function installLocalAi(item: LocalAiCatalogItem) {
    setBusy(true);
    try {
      const next = await installLocalAiModel(item.modelKey, {
        confirmNetworkDownload: true,
        confirmThirdPartyLicense: true,
        confirmSystemInstall: true,
        repair: item.status === "failed",
      });
      setLocalAiJob(next);
      setLocalAiInstallJob(null);
      setNotice(`${item.displayName} setup is running locally.`);
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
      setBusy(false);
  ...
  async function verifyLocalAi(item: LocalAiCatalogItem) {
    setBusy(true);
    try {
      await verifyLocalAiModel(item.modelKey);
      await refreshLocalAi();
      setNotice(`${item.displayName} health was checked.`);
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
      setBusy(false);
  ...
  async function resolveCleaningIssue(issue: TextCleanlinessIssue) {
    try {
      const updated = await updateCleaningIssue(issue.id, {
        status: "resolved",
        resolvedByUser: true,
      });
      setCleaningIssues((current) =>
        current.map((item) => (item.id === updated.id ? updated : item)),
      );
    } catch (cause) {
      setError(messageOf(cause));
  ...
  async function startKokoroSetup() {
    setSelectedTtsProvider("kokoro");
    setBusy(true);
    try {
      const next = await installKokoroSetup({
        confirmNetworkDownload: true,
        confirmThirdPartyLicense: true,
        repair:
          kokoroSetup?.state === "failed" ||
          kokoroSetup?.state === "incomplete",
      });
      setSetupJob(next);
      setNotice(
        "Kokoro setup is running locally. This can take several minutes on the first run.",
      );
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
  ...
  async function ensureKokoroVoice(voiceId: string) {
    ...
    const existing = voices.find(
      (voice) =>
        voice.providerVoiceId === voiceId && voice.backend === "kokoro",
    );
    if (existing) return existing;
    const created = await createVoice(selectedProjectId, {
      name: `Kokoro ${voiceId}`,
      backend: "kokoro",
  ...
  async function previewKokoroVoice(voiceId: string) {
    try {
      const voice = await ensureKokoroVoice(voiceId);
      if (voice) await playPreview(voice.id);
    } catch (cause) {
      setError(messageOf(cause));
    }
  }
  async function selectKokoroNarrator(voiceId: string) {
    try {
      const voice = await ensureKokoroVoice(voiceId);
      if (voice) await selectNarrator(voice.id);
    } catch (cause) {
      setError(messageOf(cause));
    }
  }
  ...
  async function inferDirections() {
    ...
    setBusy(true);
    try {
      const next = await inferSegmentDirections(selectedProjectId);
      await waitFor(next.id, selectedProjectId);
      setSegmentDirections(await listSegmentDirections(selectedProjectId));
      setNotice("Direction inference completed.");
      if (selectedChapter)
        await refreshProduction(selectedProjectId, selectedChapter.id);
    } catch (cause) {
      setError(messageOf(cause));
  ...
  async function addSoundCue() {
    if (
      ...
      return;
    try {
      const cue = await createSoundCue(selectedProjectId, {
        sceneId: currentSceneId,
        assetId: activeSoundAsset.id,
      ...
      setSoundCues((current) => [...current, cue]);
      setNotice(
        "Sound cue assigned. Assemble a light or dramatized mix when narration is current.",
      );
    } catch (cause) {
      setError(messageOf(cause));
    }
  ...
  async function runReadinessReport() {
    ...
    try {
      const report = await runReadiness(selectedProjectId, selectedChapter?.id);
      setReadiness(report);
      setIssues(await listIssues(selectedProjectId));
      setNotice("Readiness report updated.");
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
  ...
  async function approveSelectedChapter() {
    ...
    try {
      const approval = await approveChapter(
        selectedProjectId,
        selectedChapter.id,
        { approvedBy: "local-user" },
      );
      setChapterApproval(approval);
      setReadiness(await runReadiness(selectedProjectId, selectedChapter.id));
      setIssues(await listIssues(selectedProjectId));
      setNotice("Chapter marked listened and approved.");
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
  ...
  async function exportSelected(
    ...
    if (!selectedProjectId || !selectedExportIds.length) return;
    setSectionWorking("export", true);
    try {
      const item = await createExport(
        selectedProjectId,
        format,
        selectedExportIds,
        {
          ...
          language: exportLanguage.trim() || undefined,
          coverImagePath: exportCoverPath.trim() || undefined,
          includeRetailSample: options?.includeRetailSample,
        },
      );
      setExports((current) => [item, ...current]);
      setActiveSection("export");
      setNotice(
        "Export package created. Download the ZIP from export history.",
      );
      await refreshExportEstimate(selectedProjectId);
    } catch (cause) {
      setError(messageOf(cause));
    } finally {
      setSectionWorking("export", false);
    }
  }
  function followWorkflowAction(action: WorkflowAction) {
    setActiveSection(action.step);
    if (action.segmentId) void inspectSegment(action.segmentId);
    if (action.issueId) {
    ...
    <div className="desk-shell">
      ...
      {activeStructureJob ? (
        <InlineNotice tone="info">
          Structure & Cast Draft is running:{" "}
          {jobProgressMessage(activeStructureJob, "Working locally")}
          {activeStructureJobPercent !== null
            ? ` (${activeStructureJobPercent}%)`
            : ""}
        </InlineNotice>
      ) : null}
      {terminalStructureJob && !error ? (
        <InlineNotice tone="error">
          Last Structure & Cast Draft {terminalStructureJob.status}:{" "}
          {terminalStructureJob.errorMessage ??
            jobProgressMessage(
              terminalStructureJob,
              "Retry the workflow from the last saved artifact.",
            )}
        </InlineNotice>
      ) : null}
      <StudioShell
        ...
        nextAction={workflowActions[0] ?? null}
        onAction={followWorkflowAction}
      >
        {activeSection === "project" ? (
          <section className="workspace">
            ...
            <ProjectLibraryPanel
              ...
              onOpen={(projectId) => void loadProject(projectId)}
            />
          </section>
        ) : null}
        {project ? (
          <>
            {activeSection === "voice-engine" ? (
              <section
                className="studio-section settings"
                aria-labelledby="voice-engine-title"
              >
                <div>
                  <p className="eyebrow">01 / Voice engine</p>
                  <h2 id="voice-engine-title">Choose a voice engine</h2>
                  <p className="lede">
                    Mock is built in. Kokoro, Piper, and consent-gated XTTS-v2
                    run locally; no manuscript text or generated audio leaves
                    this machine.
                  </p>
                </div>
                <div className="studio-card">
                  <div className="field-row"><Select label="Voice engine" value={selectedTtsProvider} onValueChange={(value) => {
                    const provider = value as TtsProvider;
                    setSelectedTtsProvider(provider);
                    setTts((current) => ({ ...(current ?? emptyTts(provider)), provider, setupMode: provider === "kokoro" ? "managed_onnx" : provider === "piper" ? "local_cli" : provider === "xtts_v2" ? "coqui_local" : null }));
                  }} options={[{ value: "mock", label: "Mock (silent workflow audio)" }, { value: "kokoro", label: "Kokoro managed preset voice system" }, { value: "piper", label: "Piper local fallback" }, { value: "xtts_v2", label: "XTTS-v2 opt-in reference voice" }]} /></div>
                  <ProviderStatus
                    providers={ttsProviders}
                    active={selectedTtsProvider}
                  />
                  {selectedTtsProvider === "mock" ? (
                    <>
                      <button
                        type="button"
                        ...
                        disabled={busy}
                      >
                        {uiCopy.startWithMockVoiceEngine}
                      </button>
                      <p
                        className={
                          tts?.ready && tts.provider === "mock"
                            ? "capability ready"
                            : "capability"
                        }
                      >
                        {tts?.ready && tts.provider === "mock"
                          ? "Mock voice engine is ready. It creates silent workflow audio."
                          : tts?.message ||
                            activeProviderInfo?.message ||
                            "Mock is built in and needs no download."}
                      </p>
                    </>
                  ) : selectedTtsProvider === "kokoro" ? (
                    <div className="kokoro-setup-card">
                      <div>
                        <strong>Set up Kokoro preset voices</strong>
                        <p>
                          {kokoroSetup?.message ||
                            "Echodraft will create a local Python runtime, download Kokoro ONNX model files, build the preset voice list, and validate a local preview."}
                        </p>
                        <small>
                          Runtime:{" "}
                          {kokoroSetup?.runtimeRoot ||
                            ".echodraft/kokoro/managed-onnx-v1"}
                        </small>
                      </div>
                      <button
                        type="button"
                        onClick={() => void startKokoroSetup()}
                        disabled={busy || setupJob?.status === "running"}
                      >
                        {kokoroSetup?.state === "failed" ||
                        kokoroSetup?.state === "incomplete"
                          ? "Repair setup"
                          : kokoroSetup?.ready
                            ? "Repair setup"
                            : "Download and install Kokoro locally"}
                      </button>
                      {setupJob &&
                      ["queued", "running"].includes(setupJob.status) ? (
                        <p className="capability">
                          Step {String(setupJob.progress.step ?? "?")}/
                          {String(setupJob.progress.total ?? "?")}:{" "}
                          {String(
                            setupJob.progress.message ??
                              setupJob.progress.phase ??
                              "working",
                          )}
                        </p>
                      ) : null}
                      <div className="setup-steps">
                        {(kokoroSetup?.steps ?? []).map((step) => (
                          <span
                            key={step.phase}
                      ...
                      {kokoroVoices.length ? (
                        <div className="voice-picker">
                          <strong>Available Kokoro preset voices</strong>
                          {kokoroVoices.slice(0, 12).map((voiceId) => {
                            const profile = voices.find(
                              (voice) =>
                                voice.providerVoiceId === voiceId &&
                                voice.backend === "kokoro",
                            );
                            return (
                              <article className="voice-card" key={voiceId}>
                                <div>
                                  <strong>{voiceId}</strong>
                                  <small>
                                    {profile
                                      ? "Preset profile ready"
                                ...
                                <span>
                                  <button
                                    ...
                                    onClick={() =>
                                      void previewKokoroVoice(voiceId)
                                    }
                                  >
                                    Preview
                                  </button>
                                  <button
                                    ...
                                    className="small-button"
                                    onClick={() =>
                                      void selectKokoroNarrator(voiceId)
                                    }
                                  >
                                    {profile && narrator?.id === profile.id
                                      ? "Narrator"
                                      : "Set narrator"}
                                  </button>
                                </span>
                      ...
                      <button
                        ...
                        className="small-button"
                        onClick={() =>
                          setShowAdvancedTts((current) => !current)
                        }
                      >
                        Advanced: use a custom voice adapter
                      </button>
                      {showAdvancedTts ? (
                        <div className="advanced-tts">
                          <p className="capability">
                            Compatibility mode for a prebuilt wrapper. Most
                            users should use managed Kokoro setup above.
                          </p>
                          <label>
                            Executable
                            <input
                              value={tts?.executable ?? ""}
                              onChange={(event) =>
                                setTts((current) => ({
                                  ...(current ?? emptyTts("kokoro")),
                          ...
                          <label>
                            Voice registry
                            <input
                              value={tts?.voiceRegistryPath ?? ""}
                              onChange={(event) =>
                                setTts((current) => ({
                                  ...(current ?? emptyTts("kokoro")),
                                  setupMode: "custom_adapter",
                                  modelPath: tts?.modelPath,
                                  voiceRegistryPath: event.target.value,
                                }))
                              }
                            />
                          </label>
            ...
            {activeSection === "structure" || activeSection === "produce" ? (
              <StoryMapPanel
                ...
                supportedDirection={
                  (
                    ttsProviders.find((item) => item.provider === tts?.provider)
                      ?.capabilities as { direction?: string[] } | undefined
                  )?.direction ?? null
                }
            ...
            {activeSection === "review-patch" ? (
              <ReviewPatchPanel
                ...
                approval={chapterApproval}
                provider={tts?.provider}
                busy={Boolean(sectionBusy.readiness || sectionBusy.approval)}
            ...
            {activeSection === "export" ? (
              <ExportPanel
                ...
                onExport={(format, options) =>
                  void exportSelected(format, options)
                }
              />
            ) : null}
          </>
