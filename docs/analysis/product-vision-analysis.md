...
- Kokoro voice IDs encode locale+gender (`af_*`/`am_*`/`bf_*`/`bm_*`) and now drive initial casting suggestions; the remaining deterministic leverage is deeper acoustic/personality faceting and voice-contrast checks.
...
**Where it breaks.** Recall depends on the LLM being installed — deterministic candidates are only generated from segments that *already* have a speaker hint (`cast_discovery.py:190-227`), so a character described in narration but never tagged as speaking is invisible without Ollama. Alias linking now has a bundled nickname/title layer and conservative spelling-variant review, and exact same-name matches with conflicting observed traits are gated before merge. It is still not a full corpus-level alias graph for cases like Darcy↔Fitzwilliam. Casting traits exist for directly observed role/age/accent/gender evidence, but richer vocal descriptors are still limited. Rich evidence is computed (`_candidate_evidence_graph`) and only partially surfaced. Corrections propagate for confirmed speaker and cast decisions, but broader evidence UX and narration-only recall remain incomplete.
...
**Algorithm path.** *Deterministic (ship regardless of LLM):* full-narration proper-noun candidate generation; expand the bundled nickname/diminutive lexicon; add Jaro-Winkler + title-stripped-surname matching; alias clustering as a union-find graph over name-similarity + co-occurrence edges; extend same-name disambiguation beyond the trait gate with scene distance + co-occurring names; lightweight recency+gender coreference for pronoun tags. *LLM-assisted:* extend the extraction schema with `genderGuess`/`ageBandGuess`/`vocalDescriptor`; map-reduce the merge-verification pass (it currently dumps *all* cast + *all* candidates into one prompt — collapses on large casts); carry a rolling "known cast" summary across discovery batches; inject user-confirmed merges as few-shot exemplars.
...
**Priorities:** full corpus-level alias graph **(algorithm, M)** · richer scene/co-presence disambiguation **(algorithm, M)** · surface evidence on cards **(workflow, S)** · actionable triage queue **(workflow, M)** · vocal descriptor fields for casting **(both, M)** · full-narration candidate generation **(algorithm, M)**.

### 3.2 Manuscript understanding & structure

**Where it breaks.** The parser now preserves container chapter signals from DOCX headings and EPUB spine/TOC, classifies explicit front/back matter, keeps multi-paragraph dialogue as dialogue, routes footnote-like paragraphs to review, detects language at document and chapter level, and uses clause-aware prosody fallback for overlong narration. Remaining structure gaps are narrower but still real: numeric/roman/centered headings are limited, the clean pass can still flatten line-sensitive verse/scripts before review, duplicate-adjacent-paragraph removal can silently delete real repeated dialogue ("No." / "No."), em-dash/script nuances are incomplete, language detection does not yet drive OCR/name-matching/TTS selection, and footnote handling is text-pattern based rather than typography/page-position aware.
...
**Algorithm path.** Read container metadata as ground truth (shipped for DOCX headings and EPUB spine/TOC). Broaden heading detection + TOC cross-check as a validator that *warns on disagreement*. Continue front/back-matter classification beyond explicit headings. Extend the scene-scoped quote scanner to more em-dash and script dialogue forms. Feed document/chapter language detection into OCR `-l`, name matching, and TTS language selection. Scene breaks from three independent signal families (time / place / POV shift), not one keyword list. Continue prosody-tuned segment boundaries beyond the shipped clause-level fallback. Add positional footnote detection via the existing PDF page-render pipeline. LLM only for ambiguous heading/scene calls, on small windows, with the existing no-echo validation discipline.
...
**Priorities:** conditional/reversible line-wrap merge **(both, M)** · broaden heading detection + TOC cross-check **(algorithm, M)** · em-dash/script dialogue coverage **(algorithm, M)** · language-adaptive OCR/name/TTS behavior **(algorithm, M)** · user-directed split **(workflow, S)** · pre-reparse edit-preservation **(workflow, M/L)**.

### 3.3 Speaker attribution & dialogue

**Where it breaks.** Attribution now has conservative deterministic help for nearby turns, same-scene two-speaker alternation, speech-action cues, gendered pronoun coreference, full same-scene active-speaker rosters, interruption exchanges, vocative exchanges, and confident cast-back proposals. The LLM pass now sees bounded same-scene windows with an explicit active-speaker roster and only writes back target unresolved segment IDs. Longer group scenes and ambiguous active-speaker rosters remain review work. The review UI is a flat card grid that reshuffles as you work, each card a 160-char snippet with no audio or transcript-level scene context.
...
**Algorithm path.** Extend the conservative active-speaker model into richer group-scene uncertainty and broader coreference while keeping inferred rows review-first. Calibrate rule-specific confidence (auto-approve colon/tag matches at a lower bar; stay conservative on inference). Keep using the LLM's same-scene windows with explicit active-speaker roster evidence plus the existing few-shot examples from confirmed lines.
...
**Priorities:** scene transcript view **(workflow, M)** · group-scene uncertainty and broader coreference **(algorithm, M)** · reconcile confidence gates + re-trigger on cast change **(algorithm, S)** · bulk-approve / surface-only-ambiguous **(workflow, S)**.

### 3.4 Voice casting & performance direction

**Where it breaks.** Casting now has a first trait-ranked suggestion path: `VoiceProfile` exposes derived Kokoro locale/gender facets, suggestions compare those facets with observed character traits, and the Voice Bible can audition a suggested voice against the character's own approved dialogue. Managed Kokoro ONNX now keeps a resident local worker alive across previews/renders, so its model no longer reloads for every line. Opt-in local LLM direction inference now uses same-scene `TARGET`/`CONTEXT` windows with segment type, speaker candidate/approved attribution, mood continuity, deterministic hints, and persisted evidence. It still lacks richer acoustic/personality facets, a cast-lineup comparison, a confusable-voice check, first-class UI evidence review for direction, and XTTS style behavior.
...
**Algorithm path.** *Sequence matters: fix transmission before smarter inference, or you just make the dishonesty more sophisticated.* First: keep tightening the G4 direction path by making deterministic `_infer` dialogue/narration-aware via `segment_type`, making XTTS capability claims fully honest, and preserving audible pause/pace behavior through every render path. The first evidence-based direction backend pass now reuses the proven local LLM scene-window pattern; next it should calibrate confidence against renders and expose evidence in review workflows. Extend the completed Kokoro voice-ID facet extraction into richer acoustic/personality metadata and cast-contrast scoring. The managed Kokoro resident worker is now the first fast-audition foundation; broader engine residency can follow if needed.

**Workflow/UX.** Ranked "suggested voices" with one-click audition **playing the character's own dialogue** now exists for individual characters. The next casting UX is a "cast lineup" that plays one line across all voices back-to-back to catch confusable voices (enforcing the Voice Bible's own contrast rule). Surface real per-engine capability inline ("pace won't be audible with this voice"). In Direction Studio: a "play with current direction" button, inferred-evidence display, controls grayed out per engine capability, **batch-apply direction by scene/mood**, and a scene emotional-arc mini-timeline to spot flatlined or whiplashing delivery.

**Priorities:** fix XTTS false style claim **(algorithm, S)** · stronger `segment_type` deterministic inference **(algorithm, S)** · cast-lineup/confusable-voice checks **(workflow, M)** · direction evidence review and scene mood workflow **(workflow, M)** · deeper acoustic/personality voice facets **(algorithm, M)**.
...
**Algorithm path.** Real peak/RMS/LUFS/true-peak → inter-segment loudness-consistency + chapter-bounds checks. RMS-sliding-window dead-air detection feeding the silence metadata. Duration-vs-text truncation heuristic (wpm model per provider). Local whisper.cpp-compatible ASR word-match now verifies segment renders said the expected words when configured; next steps are timestamped/chapter-level ASR alignment, pre-render **pronunciation-risk flagging** (NER + OOV scan cross-referenced against the pronunciation dict), render-time speaker-voice consistency, and a cheap pitch/energy-variance flatness warning tier. **Auto-resolution**: re-evaluate against the newest render and system-resolve issues that now pass.
...
**Priorities:** timestamped/chapter-level ASR alignment **(algorithm, M)** · pronunciation-risk preflagging **(algorithm, M)** · render-time voice consistency **(algorithm, M)** · fix chapter/export issue scoping **(workflow, S)** · jump-to-audio waveform **(workflow, M)**.
...
- Evidence-based local LLM direction inference with character-mood continuity; persistent TTS worker.
...
- Baseline front/back-matter classification, multilingual detection, footnote routing, and prosody-tuned segmentation shipped; remaining work is language-adaptive OCR/name/TTS behavior and line-sensitive structure.
