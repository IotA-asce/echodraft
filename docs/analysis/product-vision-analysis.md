...
**Priorities:** fix XTTS false style claim **(algorithm, S)** · `segment_type`-aware inference **(algorithm, S)** · cast-lineup/confusable-voice checks **(workflow, M)** · LLM evidence-based direction **(algorithm, M)** · deeper acoustic/personality voice facets **(algorithm, M)**.
