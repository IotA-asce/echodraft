...
**Where it breaks.** Attribution now has conservative deterministic help for nearby turns, same-scene two-speaker alternation, speech-action cues, gendered pronoun coreference, and confident cast-back proposals. The LLM pass now sees bounded same-scene windows and only writes back target unresolved segment IDs. It is still not a full scene speaker-set model: interruptions, longer group scenes, and ambiguous active-speaker rosters remain review work. The review UI is a flat card grid that reshuffles as you work, each card a 160-char snippet with no audio or transcript-level scene context.
...
**Algorithm path.** Expand the initial deterministic alternation/coreference rules into a full per-scene active speaker set with interruptions, vocatives, and group-scene uncertainty. Calibrate rule-specific confidence (auto-approve colon/tag matches at a lower bar; stay conservative on inference). Build on the LLM's same-scene windows by adding explicit active-speaker roster evidence plus the existing few-shot examples from confirmed lines.
...
**Priorities:** scene transcript view **(workflow, M)** · full active-speaker model **(algorithm, M)** · active-speaker roster evidence for LLM windows **(algorithm, S/M)** · reconcile confidence gates + re-trigger on cast change **(algorithm, S)** · bulk-approve / surface-only-ambiguous **(workflow, S)**.

### 3.4 Voice casting & performance direction

**Where it breaks.** Casting now has a first trait-ranked suggestion path: `VoiceProfile` exposes derived Kokoro locale/gender facets, suggestions compare those facets with observed character traits, and the Voice Bible can audition a suggested voice against the character's own approved dialogue. It still lacks richer acoustic/personality facets, a cast-lineup comparison, and a confusable-voice check. Direction inference still casefolds the *whole segment* and substring-matches (the word "now" → urgent), blind to character, speaker, scene, continuity, and even `segment_type`. Direction transmission is no longer wholly fake after the G4 fixes, but evidence-based direction, XTTS style behavior, and persistent local model residency remain incomplete.
...
**Priorities:** fix XTTS false style claim **(algorithm, S)** · `segment_type`-aware inference **(algorithm, S)** · cast-lineup/confusable-voice checks **(workflow, M)** · LLM evidence-based direction **(algorithm, M)** · persistent TTS worker **(algorithm, M)** · deeper acoustic/personality voice facets **(algorithm, M)**.
