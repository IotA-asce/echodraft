...
**Algorithm path.** Expand the initial deterministic alternation/coreference rules into a full per-scene active speaker set with interruptions, vocatives, and group-scene uncertainty. Calibrate rule-specific confidence (auto-approve colon/tag matches at a lower bar; stay conservative on inference). Give the LLM a contiguous scene window plus the existing few-shot examples from confirmed lines.
...
**Priorities:** scene transcript view **(workflow, M)** · full active-speaker model **(algorithm, M)** · LLM scene windows **(algorithm, S/M)** · reconcile confidence gates + re-trigger on cast change **(algorithm, S)** · bulk-approve / surface-only-ambiguous **(workflow, S)**.
